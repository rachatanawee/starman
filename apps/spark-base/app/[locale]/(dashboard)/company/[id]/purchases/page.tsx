...
import { mockPurchasesData } from '@/modules/purchases'
import type { Purchases } from '@/modules/purchases'
...
export default function PurchasesPage() {
  ...
  const filteredData = React.useMemo(() => {
    return mockPurchasesData.filter(item => {
  ...
  const [data, setData] = React.useState<Purchases[]>(filteredData)
  ...
  const columns = React.useMemo<ColumnDef<Purchases>[]>(
    () => [
      {
        id: 'code',
        accessorKey: 'code',
        ...
        cell: ({ row }) => (
          <button
            onClick={() => router.push(`/${params.locale}/company/${projectId}/purchases/${row.original.id}`)}
