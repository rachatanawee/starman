export function generatePageTemplate(names: {
  kebabName: string
  camelName: string
  pascalName: string
  titleName: string
...
import { mock${pascalName}Data } from '@/modules/${kebabName}'
import type { ${pascalName} } from '@/modules/${kebabName}'
...
export default function ${pascalName}Page() {
  ...
  const filteredData = React.useMemo(() => {
    return mock${pascalName}Data.filter(item => {
  ...
  const [data, setData] = React.useState<${pascalName}[]>(filteredData)
  ...
  const columns = React.useMemo<ColumnDef<${pascalName}>[]>(
    () => [
      {
        id: 'code',
        accessorKey: 'code',
        ...
        cell: ({ row }) => (
          <button
            onClick={() => router.push(\`/\${params.locale}/company/\${projectId}/${kebabName}/\${row.original.id}\`)}
