...
import { DataTableRangeFilter } from "@/shared/components/tablecn/data-table/data-table-range-filter";
...
import { useDebouncedCallback } from "@/shared/components/tablecn/hooks/use-debounced-callback";
import { getDefaultFilterOperator, getFilterOperators } from "@/shared/components/tablecn/lib/data-table";
import { formatDate } from "@/shared/components/tablecn/lib/format";
import { generateId } from "@/shared/components/tablecn/lib/id";
import { getFiltersStateParser } from "@/shared/components/tablecn/lib/parsers";
import { cn } from "@/shared/lib/utils";
import type { ExtendedColumnFilter, FilterOperator } from "@/shared/components/tablecn/types/data-table";
...
export function DataTableFilterMenu<TData>({
  ...
  const onFilterAdd = React.useCallback(
    (column: Column<TData>, value: string) => {
      ...
      debouncedSetFilters([...(Array.isArray(filters) ? filters : []), newFilter]);
      setOpen(false);
  ...
  const onFilterRemove = React.useCallback(
    (filterId: string) => {
      const updatedFilters = Array.isArray(filters)
  ...
  const onFilterUpdate = React.useCallback(
    (
      ...
      debouncedSetFilters((prevFilters) => {
        const updatedFilters = (Array.isArray(prevFilters) ? prevFilters : []).map((filter) => {
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant="outline"
            size={Array.isArray(filters) && filters.length > 0 ? "icon" : "sm"}
            className={cn(Array.isArray(filters) && filters.length > 0 && "size-8", "h-8 font-normal")}
            ref={triggerRef}
            onKeyDown={onTriggerKeyDown}
          >
            <ListFilter className="text-muted-foreground" />
            {Array.isArray(filters) && filters.length > 0 ? null : "Filter"}
          </Button>
        </PopoverTrigger>
        <PopoverContent
          align={align}
